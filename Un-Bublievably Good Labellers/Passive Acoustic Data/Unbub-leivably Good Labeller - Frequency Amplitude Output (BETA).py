...
class SpectrogramGUI:
    def __init__(self, root, wav_path):
        ...
        self.Sxx = None
        self.t = None
        self.audio_gain = 2.598  # Gain factor for amplitude conversion
    ...
    def draw_chunk(self):
        ...
        self.Sxx = Sxx_db[lower_limit:upper_limit]
        self.t = t
    ...
    def get_bubble_bounds(self, time_bin, power_spectrum):
        i = -1
        while True:
            if power_spectrum[time_bin] > power_spectrum[time_bin + i]:
                lower_bound = int(((time_bin + i) * self.time_dif) * self.sr)
                break
            i -= 1
        ...
        while True:
            if power_spectrum[time_bin] > power_spectrum[time_bin + i]:
                upper_bound = int((((time_bin + i + 1) * self.time_dif) +self.time_dif) * self.sr)          
                break                           #+1 to include the upper bound ind
            i += 1      

        return (lower_bound, upper_bound)

    def get_power_plot(self):
        power_time = []
        _spec = self.Sxx - np.min(self.Sxx) # Normalise spectrogram to have a minimum value of 0
        for y in range(len(_spec[0])):
            _temp = []
            for x in range(len(_spec)):
                _temp.append(_spec[x][y])
            rms = np.sqrt(np.mean(np.square(_temp))) # calculate the root mean square of the time bin
            power_time.append(rms)

        return power_time


    def process_chunk_markers(self):
        chunk_powerplot = self.get_power_plot()

        for marker in self.chunk_markers:
            _t = marker.get_xdata()[0]
            bounds = self.get_bubble_bounds(int(_t / self.time_dif), chunk_powerplot)
            self.markers_dict.append({"Chunk": self.current_chunk, "Time": marker.get_xdata()[0], "Freq": marker.get_ydata()[0], "Bounds": bounds})
...
############### change to do function ###############
    def export_markers(self):
        ...
        for i in range(self.num_chunks):
            ...
            nyquist = 0.5 * self.sr
            low = self.lower_freq_limit / nyquist
            high = self.upper_freq_limit / nyquist
            b, a = butter(4, [low, high], btype='band')
            f_chunk = filtfilt(b, a, _chunk)

            for i in range(len(times)):
                time_index = int(times[i] / t_dif)- 1
                try:
                    max_freq_at_time = freqs[np.argmax(Sxx[lower_limit_index:upper_limit_index, time_index]) + lower_limit_index] + (0.5 * freq_dif)
                except:
                    print(f"Error processing chunk {i} at time {times[i]}")

                #extract section
                bubble_signal = f_chunk[int(bounds[i][0]):int(bounds[i][1])]
                peak_amp = np.max(np.abs(bubble_signal))
                amp_in_volts = (peak_amp / 32768.0) * self.audio_gain
                ...
                output.append({
                    ...
                    "Amp": amp_in_volts,
                    "Bounds": [bound / self.sr for bound in bounds[i]]
                })
